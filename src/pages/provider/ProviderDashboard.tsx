...
import { 
  ...
  ClipboardList, CheckCircle2, Clock, Building2, UserCircle, LogIn, Link as LinkIcon, UserPlus, QrCode
} from "lucide-react";
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-slate-50 text-slate-900">
      <div className="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/verify-patient"
            className="inline-flex items-center px-4 py-2 text-xs font-bold text-white bg-blue-900 hover:bg-blue-800 rounded-sm transition-colors shadow-2xs"
          >
            <QrCode className="mr-1.5 h-3.5 w-3.5" /> Scan / Verify Patient QR
          </Link>
...
function DoctorWorkspace({ doctor, onLogout }: { doctor: Doctor, onLogout: () => void }) {
  ...
  const [selectedPatient, setSelectedPatient] = useState<Patient | null>(() => {
    const qAadhaar = searchParams.get('patientAadhaar');
    if (qAadhaar) {
      return patients.find(p => p.aadhaar === qAadhaar) || null;
    }
    return null;
  });
  ...
  useEffect(() => {
    const qAadhaar = searchParams.get('patientAadhaar');
    if (qAadhaar && !selectedPatient) {
      const found = patients.find(p => p.aadhaar === qAadhaar);
          ...
          <nav className="flex-1 overflow-y-auto py-4">
            <ul className="space-y-1">
              ...
              <li className="pt-2 border-t border-slate-100">
                <Link
                  to="/verify-patient"
                  className="w-full flex items-center px-6 py-2.5 text-sm font-medium text-slate-600 hover:bg-blue-50 hover:text-blue-900 transition-colors"
                >
                  <QrCode className="mr-3 h-4 w-4 text-blue-900" /> Verify Patient QR
                </Link>
              </li>
            </ul>
          </nav>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="bg-white border border-slate-200 p-8 rounded-sm shadow-sm text-center">
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Patient Lookup & Verification</h1>
        <p className="text-sm text-slate-600 mb-6">Search CareSphere by Aadhaar Health ID, patient name, or scan their digital Health QR code.</p>
        ...
        <div className="max-w-2xl mx-auto flex flex-col sm:flex-row items-center gap-3">
          <div className="flex-1 relative w-full">
            ...
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="block w-full rounded-sm border border-slate-300 bg-slate-50 py-3 pl-12 pr-4 text-sm text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-blue-900 focus:outline-none focus:ring-1 focus:ring-blue-900 font-mono transition-colors" placeholder="ENTER AADHAAR ID OR NAME..." />
          </div>
          <Link
            to="/verify-patient"
            className="w-full sm:w-auto inline-flex items-center justify-center px-5 py-3 bg-blue-900 hover:bg-blue-800 text-white text-xs font-bold rounded-sm transition-colors shadow-2xs whitespace-nowrap"
          >
            <QrCode className="mr-2 h-4 w-4" /> Scan Patient QR
          </Link>
        </div>
